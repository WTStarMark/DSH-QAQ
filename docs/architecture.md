...
- [守卫生命周期](guard-lifecycle.md) — 一次受监督启动的完整流程
- [状态存储与回滚引擎](state-and-rollback.md) — state.json / 快照 / 防循环
- [UI 检测与 CDP](ui-detection.md) — headless Chrome + 真实 DOM 判据
- [懒人脚本控制台与环境发现](console-and-env.md) — 交互菜单 / 环境自检 / 插件挂载
- [日志系统](logging.md) — 结构化多文件轮转日志
- [测试与真实集成](testing.md) — 单测矩阵 / smoke / 真实 DSH 联调
