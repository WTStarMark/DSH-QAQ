...
> Prefer a browser GUI? Run `qaq web` (default `http://127.0.0.1:3090`) — visually consistent with the DSH web GUI (light/dark theme); start/stop the guard, backup/restore, plugin management, logs, hot update, version check.
...
| `qaq web` / `qaq serve`                     | start a local Web GUI (HTTP + WebSocket, DSH design style; default `127.0.0.1:3090`, light/dark theme)        |
