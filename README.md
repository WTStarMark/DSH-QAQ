...
> 也可以用浏览器 Web GUI：`qaq web`（默认 `http://127.0.0.1:3090`）——与 DSH Web GUI 视觉统一（浅色/深色主题），可启动/停止守卫、备份/还原、插件管理、日志、热更新、版本检测。
...
| `qaq web` / `qaq serve`                      | 打开本地 Web GUI（HTTP + WebSocket，复用 DSH 设计风格；默认 `127.0.0.1:3090`，浅色/深色主题） |
